...
class Adam:
    ...
    def __init__(self, weights, biases, weight_gradients, bias_gradients, momentum_weight_average, momentum_bias_average, rms_prop_weight_average, rms_prop_bias_average):
        ...
        self.rms_prop_weight_average = (self.rms_prop_rate * rms_prop_weight_average) + ((1 - self.rms_prop_rate) * np.square(weight_gradients))
        self.rms_prop_bias_average   = (self.rms_prop_rate * rms_prop_bias_average)   + ((1 - self.rms_prop_rate) * np.square(bias_gradients))
    ...
    def updated_parameters(self):
        ...
        updated_weights = self.weights - (self.learning_rate * self.momentum_weight_average / weight_denominator)
        updated_biases  = self.biases  - (self.learning_rate * self.momentum_bias_average   / bias_denominator)

        return [
            updated_weights,
            updated_biases,
            self.momentum_weight_average,
            self.momentum_bias_average,
            self.rms_prop_weight_average,
            self.rms_prop_bias_average
        ]
